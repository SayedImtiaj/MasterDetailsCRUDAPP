...
namespace MasterDetailsCRUDAPP
{
    public partial class Form1 : Form
    {
        ...
        void Clear()
        {
            ...
            if (dgvEnrollCourse.DataSource == null)
                dgvEnrollCourse.Rows.Clear();
            else
                dgvEnrollCourse.DataSource = (dgvEnrollCourse.DataSource as DataTable).Clone();
            inTraineeID = 0;
            btnSave.Text = "Save";
        ...
        void CourseComboBoxFill()
        {
            using (SqlConnection sqlCon = new SqlConnection(strConnectionString))
            {
                ...
                cmbCourse.ValueMember =
                    dgvcmbCourse.ValueMember = "CourseID";
                cmbCourse.DisplayMember =
                    dgvcmbCourse.DisplayMember = "Course";
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (ValidateMasterDetailForm())
            {
                ...
                {
                    sqlCon.Open();
                    foreach (DataGridViewRow dgvRow in dgvEnrollCourse.Rows)
                    {
                        if (dgvRow.IsNewRow) break;
                        else
                        {
                            SqlCommand sqlCmd = new SqlCommand("TraineeEnrollAddOrEdit", sqlCon);
                            sqlCmd.CommandType = CommandType.StoredProcedure;
                            sqlCmd.Parameters.AddWithValue("@EnrollCourseID", Convert.ToInt32(dgvRow.Cells["dgvtxtEnrollCourseID"].Value == DBNull.Value ? "0" : dgvRow.Cells["dgvtxtEnrollCourseID"].Value));
                            sqlCmd.Parameters.AddWithValue("@TraineeID", _TraineeID);
                            DateTime enrollDate;
                            if (DateTime.TryParse(dgvRow.Cells["dgvtxtEnrollDate"].Value.ToString(), out enrollDate))
                            {
                                sqlCmd.Parameters.AddWithValue("@EnrollDate", enrollDate);
                            }
                            else
                            {                               
                                sqlCmd.Parameters.AddWithValue("@EnrollDate", DBNull.Value);
                            }
                            ...
                            sqlCmd.Parameters.AddWithValue("@EnrollFee", Convert.ToInt32(dgvRow.Cells["dgvtxtEnrollFee"].Value == DBNull.Value ? "0" : dgvRow.Cells["dgvtxtEnrollFee"].Value));
        ...
        void FillTraineeDataGridView()
        {
            using (SqlConnection sqlCon = new SqlConnection(strConnectionString))
            {
                ...
                dgvTrainee.Columns["Gender"].HeaderText = "Gender";
        ...
        private void dgvTrainee_DoubleClick(object sender, EventArgs e)
        {
            if (dgvTrainee.CurrentRow.Index != -1)
            {
                ...
                using (SqlConnection sqlCon = new SqlConnection(strConnectionString))
                {
                    ...
                    dgvEnrollCourse.AutoGenerateColumns = false;
                    dgvEnrollCourse.DataSource = ds.Tables[1];
        ...
        {
            DataGridViewRow dgvRow = dgvEnrollCourse.CurrentRow;
            if (dgvRow.Cells["dgvtxtEnrollCourseID"].Value != DBNull.Value)
            {
                if (MessageBox.Show("Are You Sure to Delete this Record ?", "Master Detail CRUD", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    {
                        sqlCon.Open();
                        SqlCommand sqlCmd = new SqlCommand("EnrollCourseDelete", sqlCon);
                        sqlCmd.CommandType = CommandType.StoredProcedure;
                        sqlCmd.Parameters.AddWithValue("@EnrollCourseID", Convert.ToInt32(dgvRow.Cells["dgvtxtEnrollCourseID"].Value));
                        sqlCmd.ExecuteNonQuery();
                    }
                }
                else
